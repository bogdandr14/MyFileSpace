...
namespace MyFileSpace.Core.Specifications
{
    internal class SearchFilesSpec : Specification<StoredFile>, ISpecification<StoredFile>
    {
        ...
        {
            Query.Where(file => file.Name.Contains(filter.Name) && file.IsDeleted == false && file.AccessLevel == AccessType.Public)
        ...
        {
            Query.Where(file => file.Name.Contains(filter.Name) && file.IsDeleted == false
